...
from ...utils.fsdp_utils import load_fsdp_model, load_fsdp_submodule, offload_fsdp_model, offload_fsdp_submodule
...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def _collect_lora_weights(self) -> dict:
        """Collect LoRA weights from FSDP-wrapped transformer layers."""
        lora_weights = {}
        peft_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
        for name, submodule in self.module.named_modules():
            if not re.fullmatch(r".*layers\.\d+$", name):
                continue
            ...
            del layer_weights, layer_lora_weights
            if self.use_param_offload:
